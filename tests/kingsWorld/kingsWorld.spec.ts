...
import { BACKGROUND_COLORS, EMAIL_VIOLATIONS, PROGRESS_TEXT, TEXT_TOOLTIP, TEXT_TOOLTIPCG, UI_TEXT } from "../../src/constants/textAndColorParams";
...
test.describe("King's world test", () => {
    ...
    test.describe('Check functionality of the elements', () => {
        ...
        test('Check "hide password" button functionality', async () => {
            ...
            await expect(kingsWorld.hidePassButton).toHaveClass(/hidden/)
            await kingsWorld.hidePassButton.click()
            await expect(kingsWorld.hidePassButton).toHaveClass(/active/)
            await kingsWorld.hidePassButton.click()
            await expect(kingsWorld.hidePassButton).toHaveClass(/hidden/)
        })
        ...
        for (let country of DROPDOWN_COUNTRIES.kingsWorld){
            test(`Open coutnry dropdown and find ${country}`, async ({page}) => {
                ...
                await page.evaluate(async () => {
                    const dropdownButton = document.querySelector('label[for = "select_country"] + div.select > button.btn') as HTMLElement 
                    if (dropdownButton){
        ...
        for (let currency of DROPDOWN_CURRENCIES.kingsWorld){
            test(`Open currency dropdown and find ${currency}`, async ({page}) => {
                ...
                await page.waitForTimeout(1000)
                await page.evaluate(async () => {
                    const dropdownButton = document.querySelector('label[for = "currency_select"] + div.select > button.btn') as HTMLElement
                    if (dropdownButton){
                        dropdownButton.innerText = ''}})
    ...
    test.describe('Check text content of UI elements', () => {
        ...
        test('1st stepper text', async () => {
           ...
           expect((text).toLocaleLowerCase()).toEqual((UI_TEXT.stepperOne).toLocaleLowerCase())
        })
        ...
        test('2nd stepper text', async () => {
            ...
            expect((text).toLocaleLowerCase()).toEqual((UI_TEXT.stepperTwo).toLocaleLowerCase())
        })
        ...
        test('3rd stepper text', async () => {
            ...
            expect((text).toLocaleLowerCase()).toEqual((UI_TEXT.stepperThree).toLocaleLowerCase())
        })
        ...
        test('Why us 1st stepper text', async () => {
            ...
            const text = receivedText.replace(/\s+/g, ' ').trim()
            console.log(text)
            expect((text).toLocaleLowerCase()).toEqual((UI_TEXT.whyUsStepperOne).toLocaleLowerCase())
        })
        ...
        test('Why us 2nd stepper text', async () => {
            ...
            const text = receivedText.replace(/\s+/g, ' ').trim()
            console.log(text)
            expect((text).toLocaleLowerCase()).toEqual((UI_TEXT.whyUsStepperTwo).toLocaleLowerCase())
        })
        ...
        test('Why us 3rd stepper text', async () => {
            ...
            const text = receivedText.replace(/\s+/g, ' ').trim()
            console.log(text)
            expect((text).toLocaleLowerCase()).toEqual((UI_TEXT.whyUsStepperThree).toLocaleLowerCase())
        })
        ...
        test('How to block text', async () => {
            ...
            const text = receivedText.replace(/\s+/g, ' ').trim()
            console.log(text)
            expect((text).toLocaleLowerCase()).toEqual((UI_TEXT.howToBlock).toLocaleLowerCase())
        })
        ...
        test('Check text of the terms and conditions block', async () => {
            ...
            const text = receivedText.replace(/\s+/g, ' ').trim()
            console.log(text)
            expect((text).toLocaleLowerCase()).toEqual((UI_TEXT.termsAndConditionsBlock).toLocaleLowerCase())
        })
        ...
        test('Check text of the license in the footer', async () => {
            ...
            const text = receivedText.replace(/\s+/g, ' ').trim()
            console.log(text)
            expect((text).toLocaleLowerCase()).toEqual((UI_TEXT.license).toLocaleLowerCase())
        })
    })
